...
## The repo lifecycle (grow → replant → consolidate → expand)
...
- when the lineage reaches `consolidate_at_members` (default **7**) **and the final member has finished its own generation** (it grows fully, then consolidates instead of replanting — no half-grown member, no 8th repo), `/consolidate` — run from the newest member — merges all members into one range-named repo (e.g. `2005-2011`), preserves every Evolution Log, and archives the members;
- **consolidation is not terminal** — the consolidated repo's status `consolidated` gates to `/expand`, which deepens it to month granularity (`<year>/<MM-month>/`) and once plants the next era's lineage (`policy.succession.next_lineage`, e.g. "the year 1776"). Each consolidation deepens its own range **and** births a new recursive lineage → ever-expanding knowledge bases by year.
...
| `/consolidate [--dry-run]` | Merge a completed 7-member lineage into one range-named repo and archive the members. Run from the newest member. Primes the consolidated repo to `expand`. |
| `/expand [--no-plant]` | Post-consolidation growth (consolidation is **not terminal**): deepen the consolidated repo to month granularity (`<year>/<MM-month>/`) and once plant the next era's lineage (`policy.succession.next_lineage`). Triggered by the lifecycle gate when status is `consolidated`. |
