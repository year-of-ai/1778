...
2005 🌳 ─▶ 2006 🌳 ─▶ 2007 🌳 ─▶ 2008 🌳 ─▶ 2009 🌳 ─▶ 2010 🌳 ─▶ 2011 (7th member) ──▶ /consolidate ──▶ 2005-2011 📦
                                                                                          members archived, logs preserved
                                                                                                  │
                                              ┌───── /expand (consolidation is NOT terminal) ─────┤
                                              ▼                                                    ▼
                                  deepen 2005-2011 into months                        plant next lineage: 1776 🌱
                                  (2005/01-january/, …) — ever-expanding               ──▶ grow → replant → consolidate
                                                                                       ──▶ /expand ──▶ 1776-1782 📦 ──▶ plant 1865 🌱 …  (recursion)
```
...
| **Consolidate** | `/consolidate` | lineage reaches `consolidate_at_members` (default **7**) **and the final member has finished its generation** (so it consolidates instead of replanting — no 8th member) | Merges all member repos into one repo named for the range (e.g. `2005-2011`), preserves every Evolution Log, archives the members, and **primes it to expand** (status `consolidated`, `granularity: month`). |
| **Expand** | `/expand` | repo status is `consolidated` (so consolidation is **not terminal**) | The consolidated repo keeps growing at a finer time granularity — deepens content into `<year>/<MM-month>/` structure (`policy.consolidation.deepen_granularity`) — and **once** plants the next era's lineage (`policy.succession.next_lineage`, e.g. "the year 1776") from the seed package. Each consolidation thereby deepens its own range *and* births a new recursive lineage → ever-expanding knowledge bases. |
...
**and that final member has finished its own growth generation** — it grows its full 3 ticks like
every other member, then consolidates *instead of* replanting (so no 8th member is spawned and no
member enters the consolidated repo half-grown).
